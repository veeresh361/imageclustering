...
def create_variables(name, shape, initializer=tf.initializers.GlorotUniform()):
...
def batch_normalization_layer(input_layer, dimension, phase_train, decay = 0.90):
    ...
    def mean_var_with_update():
        ema_mean_op = tf.compat.v1.assign(ema_mean, ema_mean * decay + batch_mean * (1 - decay))
        ema_var_op = tf.compat.v1.assign(ema_var, ema_var * decay + batch_var * (1 - decay))
...
def main():
    unlabeled_x = tf.compat.v1.placeholder(tf.float32, shape=[None, input_dim])
